...
fn run_tui(rx: mpsc::Receiver<String>) -> anyhow::Result<()> {
    ...
    loop {
        ...
        // Draw UI
        terminal.draw(|f| {
            // Create minimal outer padding and then reduce it
            let outer_chunks = Layout::default()
                .direction(Direction::Vertical)
                .constraints([
                    Constraint::Length(1), // Top padding: start with 2, cut in half = 1
                    Constraint::Min(0),    // Main content area
                    Constraint::Length(1), // Bottom padding: start with 2, cut in half = 1
                ].as_ref())
                .split(f.size());
            ...
            let chunks = Layout::default()
                ...
                .constraints([
                    Constraint::Length(2), // Left padding: start with 3, reduce by 1/3 = 2
                    Constraint::Min(0),    // Main content area
                    Constraint::Length(2), // Right padding: start with 3, reduce by 1/3 = 2
                ].as_ref())
