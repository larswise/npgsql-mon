    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let inner_area = ratatui::layout::Rect {
                ...
                y: chunks[0].y + 3,      // Add more vertical padding inside border (full line)
                width: chunks[0].width.saturating_sub(4),  // Reduce width for padding
                height: chunks[0].height.saturating_sub(4), // Reduce height for padding
            };
                ...
                if key_event.kind == KeyEventKind::Press {
                            ...
                            if let Some(selected) = list_state.selected() {
                                ...
                                if selected < log_lines.len() {
                                    ...
                                    if line.statement.contains("[-- Batch Command") {
                                        ...
                                        if formatted_sql.trim().is_empty() {
                                            total_lines += line.statement.lines().count().max(1);
                                        } else {
                                            total_lines += formatted_sql.lines().count();
                                        }
                    ...
                    match key.code {
                        ...
                        KeyCode::Down | KeyCode::Char('j') => {
                            if let Some(selected) = list_state.selected() {
                                if selected < log_lines.len().saturating_sub(1) {
                                    list_state.select(Some(selected + 1));
                                }
                        ...
                        KeyCode::Enter => {
                            if let Some(selected) = list_state.selected() {
                                if expanded_items.contains(&selected) {
                                    // Collapse the accordion
                                    expanded_items.remove(&selected);
                                } else {
                                    // Expand the accordion
                                    expanded_items.insert(selected);
                                }
                            }
                        }
                        KeyCode::Char('l') => {
                            if let Some(selected) = list_state.selected() {
                                if expanded_items.contains(&selected) {
                                    ...
                                    scroll_cursors.insert(selected, 0); // Start cursor at first line
                                }
                            }
                        }
                        _ => {}
